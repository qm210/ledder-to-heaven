...
def generate_mapping():
    # problem is that the WLED messaging quite stumpfly ignores the current mapping,
    # i.e. assigning the colors to any gaps that appear. We have to umrechnerize.
    result = {}
    for ix in range(LOGO_W):
        for iy in range(LOGO_H + BASE_L):
            i = ix + iy * LOGO_W
            index = mapping[i]
            if index == __:
                continue
